...
const CLUBS = [
  ...
  { id: 2, name: 'FC Barcelona', logo: '🔵' },
  { id: 3, name: 'Manchester United', logo: '🔴' },
  { id: 4, name: 'Liverpool', logo: '🔵' },
  { id: 5, name: 'Bayern Munich', logo: '🔴' },
  { id: 6, name: 'AC Milan', logo: '⚪' },
  { id: 7, name: 'Juventus', logo: '⚫' },
  { id: 8, name: 'Inter Milan', logo: '🔵' },
  { id: 9, name: 'Chelsea', logo: '🟣' },
  { id: 10, name: 'Arsenal', logo: '🟤' },
  { id: 11, name: 'Manchester City', logo: '🔵' },
  { id: 12, name: 'Paris Saint-Germain', logo: '⚪' },
  { id: 13, name: 'Borussia Dortmund', logo: '🔴' },
  { id: 14, name: 'Atlético Madrid', logo: '🟠' },
  { id: 15, name: 'Tottenham Hotspur', logo: '🔵' },
  { id: 16, name: 'Ajax', logo: '🔵' },
  { id: 17, name: 'Benfica', logo: '⚪' },
  { id: 18, name: 'FC Porto', logo: '🔴' },
  { id: 19, name: 'AS Roma', logo: '🔴' },
...
function ClubSelection({ totalPlayers = 2, onBack, onComplete }) {
  ...
  const isClubTaken = (clubId) => {
    return Object.entries(selectedClubs).some(([player, club]) => 
      club === clubId && parseInt(player) !== currentPlayer
    );
  };

  const getTakenByPlayer = (clubId) => {
    const entry = Object.entries(selectedClubs).find(([player, club]) => 
      club === clubId && parseInt(player) !== currentPlayer
    );
  ...
  return (
    <div className="club-selection">
      <div className="page-header">
        ...
        <p className="page-subtitle">Each player must select their favorite football club</p>
        <p className="current-player">Player {currentPlayer} - Make Your Choice</p>
      </div>
      ...
      <div className="progress-indicator">
        ...
        <span>{currentPlayer - 1} of {totalPlayers} players completed</span>
      </div>
      ...
      <div className="clubs-grid">
        {CLUBS.map((club) => {
          ...
          return (
            <div
              key={club.id}
              className={`club-option ${isSelected ? 'selected' : ''} ${isTaken ? 'disabled' : ''}`}
